...
func (tc *TodoCreate) SetDetail(s string) *TodoCreate {
...
func (tc *TodoCreate) SetCompletedAt(t time.Time) *TodoCreate {
	tc.mutation.SetCompletedAt(t)
	return tc
}

// SetNillableCompletedAt sets the "completed_at" field if the given value is not nil.
func (tc *TodoCreate) SetNillableCompletedAt(t *time.Time) *TodoCreate {
	if t != nil {
		tc.SetCompletedAt(*t)
	}
	return tc
...
func (tc *TodoCreate) check() error {
	...
	if _, ok := tc.mutation.Detail(); !ok {
		return &ValidationError{Name: "detail", err: errors.New(`ent: missing required field "Todo.detail"`)}
	}
	if _, ok := tc.mutation.CreatedAt(); !ok {
